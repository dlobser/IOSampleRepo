...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    private void SetPagePositions() {
        ...
        if (_horizontal) {
            ...
            float initialPosition = width / (float)_pageCount;
            initialPosition += (float)startingPage * initialPosition;
            Debug.Log(width);
            offsetX = (width/2) * _pageCount - ((int)startingPage * (width)); //(int)initialPosition;// / 2;
            // total width
